...
public class Admin {
    ...
    private Checking createChecking(BigDecimal balance, String primaryOwner, String secretKey, AccountStatus status){
        return new Checking(balance, primaryOwner, secretKey,status);
    }

    private Checking createChecking(BigDecimal balance, String primaryOwner, String secondaryOwner, String secretKey, AccountStatus status){
        return new Checking(balance, primaryOwner, secondaryOwner, secretKey, status);
    }

    private Savings createSaving(BigDecimal balance, String primaryOwner, String secretKey, BigDecimal minimumBalance, AccountStatus status, BigDecimal interestRate){
        return new Savings(balance,primaryOwner,secretKey,minimumBalance,status,interestRate);
    }

    private Savings createSaving(BigDecimal balance, String primaryOwner, String secondaryOwner, String secretKey, BigDecimal minimumBalance, AccountStatus status, BigDecimal interestRate){
